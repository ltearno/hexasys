...
/* creates a new root record and returns the node 'l'=1, 'r'=2. */
{
  $newnode['l'] = 1;
  $newnode['r'] = 2;
  $newnode['id'] = _insertNew ($thandle, $newnode, $othercols);
  return $newnode;
}
...
{ //print("SHIFT: add $delta to gr-eq than $first <br/>");
  mysql_query("UPDATE ".$thandle['table']." SET ".$thandle['lvalname']."=".$thandle['lvalname']."+$delta WHERE ".$thandle['lvalname'].">=$first", $thandle['dbconnection']);
  mysql_query("UPDATE ".$thandle['table']." SET ".$thandle['rvalname']."=".$thandle['rvalname']."+$delta WHERE ".$thandle['rvalname'].">=$first", $thandle['dbconnection']);
}
 ...
 */
{
  mysql_query("UPDATE ".$thandle['table']." SET ".$thandle['lvalname']."=".$thandle['lvalname']."+$delta WHERE ".$thandle['lvalname'].">=$first AND ".$thandle['lvalname']."<=$last", $thandle['dbconnection']);
  mysql_query("UPDATE ".$thandle['table']." SET ".$thandle['rvalname']."=".$thandle['rvalname']."+$delta WHERE ".$thandle['rvalname'].">=$first AND ".$thandle['rvalname']."<=$last", $thandle['dbconnection']);
...
/* creates a new root record and returns the node 'l'=1, 'r'=2. */
{
  ...
  $res = mysql_query("INSERT INTO ".$thandle['table']." SET $othercols"
		 .$thandle['lvalname']."=".$node['l'].", ".$thandle['rvalname']."=".$node['r'], $thandle['dbconnection']);
  if (!$res) {_prtError($thandle);}
  
  return mysql_insert_id( $thandle['dbconnection'] );
}
...
{
  $res = mysql_query("DELETE FROM ".$thandle['table'], $thandle['dbconnection']);
  if (!$res) {_prtError($thandle);}
}
...
/* deletes the node '$node' and all its children (subtree). */
{
  ...
  $res = mysql_query("DELETE FROM ".$thandle['table']." WHERE "
		 .$thandle['lvalname'].">=".$node['l']." AND ".$thandle['rvalname']."<=".$node['r'], $thandle['dbconnection']);
 ...
 */
{
  $noderes['l'] = 0;
  $noderes['r'] = 0;
  $res = mysql_query("SELECT * FROM ".$thandle['table']." WHERE ".$whereclause, $thandle['dbconnection']);
  if (!$res) {_prtError($thandle);}
  else{
    if ($row = mysql_fetch_array ($res)) {
	  ...
	  $noderes['id'] = $row['id'];
	  $noderes['type'] = $row['type'];
	  $noderes['data'] = $row['data'];
	}
	else
		$noderes = null;
  }
...
function nstNodeRoot ($thandle, $node )
{ 
	//return nstGetNodeWhere ($thandle, $thandle['lvalname']."=1");
	return nstGetNodeWhere ($thandle, 
                    $thandle['lvalname']."<=".($node['l'])
		   ." AND ".$thandle['rvalname'].">=".($node['r'])
		   ." AND ".$thandle['lvalname']."<>"."1"
...
{ 
  $res = mysql_query("SELECT COUNT(*) AS level FROM ".$thandle['table']." WHERE "
                   .$thandle['lvalname']."<".($node['l'])
		   ." AND ".$thandle['rvalname'].">".($node['r']), $thandle['dbconnection'] );
...
/* initializes preorder walk and returns a walk handle */
{
  $res = mysql_query("SELECT * FROM ".$thandle['table']
         ...
         ."   AND ".$thandle['rvalname']."<=".$node['r']
         ." ORDER BY ".$thandle['lvalname'], $thandle['dbconnection']);
...
{
  $res = mysql_query("SELECT * FROM ".$thandle['table']." WHERE ".$thandle['lvalname']."=".$node['l'], $thandle['dbconnection']);
  if ($row = mysql_fetch_array ($res)) {
...
function nstSetNodeAttribute ($thandle, $node, $attribute, $value)
{
  mysql_query("UPDATE ".$thandle['table']." SET ".$attribute."=".$value . " WHERE ".$thandle['lvalname']."=".$node['l'], $thandle['dbconnection']);
}

function nstSetNodeAttributes($thandle, $node, $attributes)
{
	$atts = array();
	foreach( $attributes as $name => $value )
		$atts[] = $name . "=" . $value;
	$attributes = implode( ', ', $atts );
  mysql_query("UPDATE ".$thandle['table']." SET ". $attributes . " WHERE ".$thandle['lvalname']."=".$node['l'], $thandle['dbconnection']);
}
...
/*  */
{
  $res = mysql_query("SELECT * FROM ".$thandle['table']." ORDER BY ".$thandle['lvalname'], $thandle['dbconnection']);
  if (!$res) {_prtError($thandle);}
  else{
    $level = -1;
	$prevl = 0;
...
function _prtError($thandle){
  echo "<p>Error: ".mysql_errno($thandle['dbconnection']).": ".mysql_error($thandle['dbconnection'])."</p>";
}
?>
